...
app.post('/pacientes', (req,res)=>{
    ...
    res.status(201).json({mensagem:"Paciente Cadastrado",paciente:novoPaciente})
})
app.put('/pacientes/:id', (req,res) => {
  const id = parseInt(req.params.id)

  const paciente_index = pacientes.findIndex(p => p.id === id)

  if(paciente_index === -1){
    return res.status(404).json({mensagem:"Usuário não encontrado"})
  }
  pacientes[paciente_index] = {
    ...pacientes[paciente_index],
    nomePaciente:req.body.nomePaciente
  }
  res.status(200).json({mensagem:"Paciente atualizado", pacientes:pacientes[paciente_index]})
})
...
app.delete('/pacientes/:id', (req,res) => {
  ...
  const removido = pacientes.splice(paciente_index,1)
  res.status(200).json({mensagem:'Usuário removido com sucesso', pacientes:removido[0]})
})
